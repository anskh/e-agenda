...
if ($model->isEdit()) {
    $action = route('edit_eksternal_keluar') . $model->id;
} else {
    $action = route('entri_eksternal_keluar');
}
?>
<div class="content-body">
    <div class="card mb-5 col-xl-8">
        ...
        <div class="card-body">
            <h4 class="card-title py-1 mb-5"><?= $model->isEdit() ? 'Ubah ' : 'Tambah ' ?>Undangan Eksternal Keluar</h4>
            <?php
            if ($model->hasError()) { ?>
                ...
                <div class="mb-3 col">
                    <label class="text-muted" for="file_input"><?= $model->getLabel('file') ?>
                        <?php if ($model->isEdit() && $model->file) { ?>
                            : <a target="_blank" href="<?= upload("/{$tahun}/eksternal_keluar/" . $model->file) ?>"><i class="fa-solid fa-file-pdf"></i></a>
                        <?php } ?>
                    </label>
        ...
        <div class="card-footer text-end">
            ...
            <a href="<?= route('eksternal_keluar') ?>" class="btn btn-secondary rounded-pill py-2 px-4">Batal</a>
            <!-- button simpan data -->
