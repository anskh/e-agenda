...
if ($model->isEdit()) {
    $action = route('edit_nota_keluar') . $model->id;
} else {
    $action = route('entri_nota_keluar');
}
?>
<div class="content-body">
    <div class="card mb-5 col-xl-8">
        ...
        <div class="card-body">
            <h4 class="card-title py-1 mb-5"><?= $model->isEdit() ? 'Ubah ' : 'Tambah ' ?>Nota Dinas/KAK/Form Belanja Keluar</h4>
            <?php
            if ($model->hasError()) { ?>
                ...
                <div class="mb-3 col">
                    <label class="text-muted" for="file_input"><?= $model->getLabel('file') ?>
                        <?php if ($model->isEdit() && $model->file) { ?>
                            : <a target="_blank" href="<?= upload("/{$tahun}/nota_keluar/" . $model->file) ?>"><i class="fa-solid fa-file-pdf"></i></a>
                        <?php } ?>
                    </label>
        ...
        <div class="card-footer text-end">
            ...
            <a href="<?= route('nota_keluar') ?>" class="btn btn-secondary rounded-pill py-2 px-4">Batal</a>
            <!-- button simpan data -->
