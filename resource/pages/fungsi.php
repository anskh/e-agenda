...
<div class="content-header">
    <h4 class="content-title">
        ...
        <span>Master Bagian/Fungsi</span>
    </h4>
</div>
<div class="content-body">
    ...
    <div class="card mb-4">
        <div class="card-header bg-white border-bottom">
            <div>
                <h5 class="card-title mb-0">Daftar Bagian/Fungsi</h5>
            </div>
            ...
            <?php if(auth()->hasPermission('entri_fungsi')) { ?>
            <div>
                <a href="<?= route('entri_fungsi') ?>" class="btn btn-primary"><i class="fa-solid fa-plus me-2"></i>Add</a>
            </div>
            <?php } ?>
        </div>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= $row['nama'] ?></td>
                            <?php if (auth()->hasPermission('edit_fungsi') && auth()->hasPermission('delete_fungsi')) { ?>
                                <td width="210" class="text-center">
                                    <div>
                                        <!-- button form ubah data -->
                                        <a href="<?= base_url('/fungsi/edit/' . $row['kode']) ?>" class="btn btn-success btn-xs"> <i class="fa-solid fa-pencil me-1"></i> Ubah </a>
                                        <!-- button modal hapus data -->
                                        <button type="button" class="btn btn-danger btn-xs me-1" data-bs-toggle="modal" data-bs-target="#modalHapus<?= $no ?>"> <i class="fa-solid fa-trash-can me-1"></i> Hapus </button>
                                        <!-- Modal hapus data -->
                                                ...
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h1 class="modal-title fs-5" id="exampleModalLabel">
                                                            <i class="fa-regular fa-trash-can me-2"></i> Hapus Data Fungsi
                                                        </h1>
                                                    </div>
                                                    <div class="modal-body text-start">
                                                        <p class="mb-2">Anda yakin ingin menghapus data fungsi?</p>
                                                        <!-- informasi data yang akan dihapus -->
                                                        <p class="fw-bold mb-2"><?= $row['kode'] ?> <span class="fw-normal">-</span> <?= $row['nama'] ?></p>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary rounded-pill px-3 me-1" data-bs-dismiss="modal">Batal</button>
                                                        <!-- button proses hapus data -->
                                                        <a href="<?= base_url('/fungsi/delete/' . $row['kode'])  ?>" class="btn btn-danger rounded-pill px-3">Ya, Hapus</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
